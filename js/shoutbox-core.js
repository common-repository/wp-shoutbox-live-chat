...
var Shoutbox = jQuery.extend(Shoutbox || {}, {
    ...
    single_message_html: function(single_message, avatars, sys_mes){
        ...
        for (var smile in Shoutbox['smilies']){
            var replace = '<img src="'+Shoutbox['smilies'][smile]+'" alt="'+Shoutbox['smilies'][smile]+'"/>'
            ...
            function(data){
                ...
                if(data.success == 1){
                    ...
                    jQuery('div.shoutbox-container').each(function(){
                        ...
                        jQuery(history_container).find('.shout-temp').remove();
                    });
    ...
    new_message: function(chat_id, message_text, sys_mes){
        ...
        var chatbox = jQuery('div.shoutbox-container[data-shoutbox-id="'+chat_id+'"] .shoutbox-history-container');
        jQuery(chatbox).append(
            jQuery(Shoutbox.single_message_html(
            {
                alias: Shoutbox.user_name,
                sys_mes:sys_mes,
                message: message_text,
                room: room_name,
                timestring: 'Только что'
            },
            0, false)).addClass('shout-temp')
        );
